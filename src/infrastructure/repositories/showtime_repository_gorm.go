...
func (repository *ShowTimeRepositoryGorm) Update(ctx context.Context, id uint, show dto.ShowDTO) (int, error) {
	...
	return gorm.G[entities.ShowTime](db).Where("id = ?", id).Updates(ctx, entities.ShowTime{StartDate: startDate,
		EndDate: endDate, AvailableQuantitySeats: show.AvailableQuantitySeats, MovieID: show.MovieID})
}
